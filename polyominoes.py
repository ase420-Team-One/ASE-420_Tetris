...
class Mino_Type_List:
    """
    To extend, just do _types + (new tuple list).       This extension is innefective, I would have to turn it into a list, then back to a tuple 
...
class Tetrimino_Type_List(Mino_Type_List):
    def __init__(self):
        self._types = (
            ((1, 5, 9, 13), (4, 5, 6, 7)),
            ((4, 5, 9, 10), (2, 6, 5, 9)),
...
class Tetrimino(Mino): # Now also changes to polyomino list.
    ...
    def __init__(self,mino):
        ...
        else: self._type_set_list = Polyomino_Type_List()
        self._previous = None
